...
2650                     ; 88     FlashProgramBlock(Block, FLASH_CODE, FLASH_PROGRAMMODE_STANDARD, GBuffer); //写一个块128字节
...
2942                     ; 161 }
2946  00a1 87            	retf
2985                     ; 171 void FlashWriteByte(u32 Address,u8 Data)
2985                     ; 172 {
2986                     	switch	.iapcode
2987  00a2               f_FlashWriteByte:
2989       00000000      OFST:	set	0
2992                     ; 173     *((@far u8*)Address) = Data; //写一个字节
...
3002                     ; 174     FlashWaitForLastOperation(); //等待写操作结束
3004  00b2 8d940094      	callf	f_FlashWaitForLastOperation
3006                     ; 175 }
3009  00b6 87            	retf
3049                     ; 185 void FlashWriteWord(u32 Address,u32 Data32)
3049                     ; 186 {
3050                     	switch	.iapcode
3051  00b7               f_FlashWriteWord:
3053       00000000      OFST:	set	0
3056                     ; 187     FLASH.CR2 |=FLASH_CR2_WPRG;
3058  00b7 721c505b      	bset	_FLASH+1,#6
3059                     ; 188     FLASH.NCR2 &=(u8)(~FLASH_NCR2_NWPRG);
3061  00bb 721d505c      	bres	_FLASH+2,#6
3062                     ; 190     *(((@far u8*)Address)+0) =BYTE_3(Data32); //写一个字节
...
3072                     ; 191     *(((@far u8*)Address)+1) =BYTE_2(Data32); //写一个字节
...
3084                     ; 192     *(((@far u8*)Address)+2) =BYTE_1(Data32); //写一个字节
...
3096                     ; 193     *(((@far u8*)Address)+3) =BYTE_0(Data32); //写一个字节
...
3109                     ; 194     FlashWaitForLastOperation(); //等待写操作结束
3111  0110 8d940094      	callf	f_FlashWaitForLastOperation
3113                     ; 195 }
3116  0114 87            	retf
3147                     ; 205 u8 FlashReadByte(u32 Address)
3147                     ; 206 {
3148                     	switch	.iapcode
3149  0115               f_FlashReadByte:
3151       00000000      OFST:	set	0
3154                     ; 207     return(*((@far u8*)Address)); //读一个字节
...
3257                     ; 234     FlashWaitForLastOperation(); //等待写操作结束
3259  013a 8d940094      	callf	f_FlashWaitForLastOperation
3261                     ; 235 }
3264  013e 87            	retf
3321                     ; 245 void FlashEraseBlock(u16 BlockNum, FlashType MemType)
3321                     ; 246 {
3322                     	switch	.iapcode
...
3372                     ; 259     FLASH.CR2  |= FLASH_CR2_ERASE;
3374  017e 721a505b      	bset	_FLASH+1,#5
3375                     ; 260     FLASH.NCR2 &= (u8)(~FLASH_NCR2_NERASE);
3377  0182 721b505c      	bres	_FLASH+2,#5
3378                     ; 261     for (i = 0; i <FLASH_BLOCK_SIZE; i++)
3380  0186 0f05          	clr	(OFST+0,sp)
3381  0188               L5671:
3382                     ; 263         *(((@far u8*)StartAddress)+i) =0x00;
...
3394                     ; 261     for (i = 0; i <FLASH_BLOCK_SIZE; i++)
3396  019c 0c05          	inc	(OFST+0,sp)
3399  019e 7b05          	ld	a,(OFST+0,sp)
3400  01a0 a180          	cp	a,#128
3401  01a2 25e4          	jrult	L5671
3402                     ; 265     FlashWaitForLastOperation(); //等待写操作结束
3404  01a4 8d940094      	callf	f_FlashWaitForLastOperation
3406                     ; 266 }
3409  01a8 5b07          	addw	sp,#7
3410  01aa 87            	retf
3509                     ; 276 void FlashProgramBlock(u16 BlockNum, FlashType MemType, FLASH_ProgramMode_TypeDef ProgMode, u8 *Buffer)
3509                     ; 277 {
3510                     	switch	.iapcode
...
3540  01ca               L7302:
3541                     ; 287         StartAddress = FLASH_DATA_START_PHYSICAL_ADDRESS;
3543  01ca ae4000        	ldw	x,#16384
3544  01cd 1f03          	ldw	(OFST-3,sp),x
3545  01cf ae0000        	ldw	x,#0
3546  01d2 1f01          	ldw	(OFST-5,sp),x
3547  01d4               L1402:
3548                     ; 291     StartAddress = StartAddress + ((u32)BlockNum *FLASH_BLOCK_SIZE);
...
3558                     ; 294     if (FLASH_PROGRAMMODE_STANDARD==ProgMode)
3560  01e4 0d0d          	tnz	(OFST+7,sp)
3561  01e6 260a          	jrne	L3402
3562                     ; 297         FLASH.CR2 |= FLASH_CR2_PRG;
3564  01e8 7210505b      	bset	_FLASH+1,#0
3565                     ; 298         FLASH.NCR2 &= (u8)(~FLASH_NCR2_NPRG);
3567  01ec 7211505c      	bres	_FLASH+2,#0
3569  01f0 2008          	jra	L5402
3570  01f2               L3402:
3571                     ; 303         FLASH.CR2 |= FLASH_CR2_FPRG;
3573  01f2 7218505b      	bset	_FLASH+1,#4
3574                     ; 304         FLASH.NCR2 &= (u8)(~FLASH_NCR2_NFPRG);
3576  01f6 7219505c      	bres	_FLASH+2,#4
3577  01fa               L5402:
3578                     ; 308     for (i = 0; i <FLASH_BLOCK_SIZE; i++)
3580  01fa 5f            	clrw	x
3581  01fb 1f05          	ldw	(OFST-1,sp),x
3582  01fd               L7402:
3583                     ; 310         *(((@far u8*)StartAddress)+i) =((u8)(Buffer[i]));
...
3597                     ; 308     for (i = 0; i <FLASH_BLOCK_SIZE; i++)
...
3607                     ; 312     FlashWaitForLastOperation(); //等待写操作结束
